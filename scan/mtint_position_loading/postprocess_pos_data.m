function [xy, dir, speed, times, jumpyPercent, n_leds, dir_disp] = postprocess_pos_data(posdata, max_speed, box_car, setfile_header, varargin)
...
% --- PARSE ARGS --------------------------------------------------------------------
%Final arg headPos, which is only relevant to 2 led situation, does not need to
%be specified (default value is 0.5) if it is specfied (i.e. 5 args in) need to
%extract it from varargin
if nargin==4 %headPos not specified
    headPos=0.5; %Default value - head is equidistant between LEDs
elseif nargin==5 %headPos specified
    headPos=varargin{1};
    if headPos<0 || headPos>1
        error('postprocess_pos_data.m: headPos must be a number >=0 and <=1./n');
    end
    clear varargin
else 
    error('postprocess_pos_data.m: Incorrect number of arguments, specify 4 or 5.\n');
end
