function [L_Ratio, IsoD] = spk_mahaldist(amps,cellIndex)
% Calculate the quality of cluster separation on a tetrode, using mahalanobis distance
% based measures. (L-Ratio of Schmitzer-Torbert & Redish, and the Isolation Distance
% of Harris, see Schmitzer-Torbert et al, 2005. Neuroscience 131).
%
%       [LR, IsoD] = spk_mahaldist(amps,cellIndex)
%
% Inputs - amps is a [4,nSpike] array, containing the spike amplitude for
%          every spike on the tetrode, on each wire of the tetrode.
%        - cellIndex is an [1,nSpike] index vector to show which spikes belong to which cells.
%
...
%
% LR and IsoD will be vectors, one value for each cell (but not cell 0).
%
...
for ii=1:length(cellList)
    ...
    IsoD(ii) = ClusterQuality(amps,currCellInd,dists);
    %-------------------------------------------
    % End of MClust code
