function [spike_mat,spike_count] = extract_DS2_from_egf(eeg_data,samp_rate,spikeThreshold)
...
% INPUT:
% egf_data: 32*samples matrix of voltage data from all channels in sleep
% trial
...
% TO DO: 1) consider making this a subfunction of get_DS2. 2) find best
% options for artifact rejection - add min and max spike width using
% distance betwee minima at either side of spike? 
...
while thr_cr_idx <= length(thr_cr_numind) 
    ...
    if thr_cr_samp > nSamp-trace_window(2) || thr_cr_samp <= trace_window(2)  % If we have reached the end of the data or we are too close to the beginning
        ...
        if  passes_waveform_shape_filter && mean(first_samps) < max_val*0.30  &&  mean(last_samps) < max_val*0.30 && spread > 0.7 % This is artefact rejection test - can modify these numbers            
...
matrix_spike_mat = zeros(32, 675, spike_count);
...
for i = 1:spike_count
    ...
    matrix_spike_mat(:, 1, i) = repmat(timestamp, 32, 1);
    matrix_spike_mat(:, 2, i) = repmat(spread, 32, 1);
