function [max_amplitude, mean_amplitude, fig1, fig2, ds2_rate, peak_to_trough_amplitude, peak_to_trough_slope, spike_mat]= get_DS2_from_egf(egf_data, probe_type, sws_table) 
...
%OUTPUTS: useful features for DS2 analysis. 
    ...
    %with change after you troubleshoot extraction (and change below in a
    %place that makes sense) 
    best_DS2s = 10; %number of DS2s in spike_mat you want to include in DS2 info summary table (can change to all by becoming size(spike_mat,3) and being placed after spike_mat is made
...
%         filtObj       = designfilt( 'highpassfir','FilterOrder',filtOrder, 'PassbandFrequency', 20, 'StopbandFrequency', 10, 'SampleRate', 4800);       
    ...
    if ~isempty(spike_mat) 
        ...
        elseif probe_type == 4 
            % gather the channels that the EEGs come
            %from - these are saved in sleep table now 
            eeg_chans = sleepData.eeg_channels; %maybe can use these to color the plots 
            probe_data = [];
        end
        ...
        if size(spike_mat,3) >= best_DS2s  %if extract_DS2 doesnt find any spikes that trial is skipped      
            ...
            for jj = 1:spikes_to_plot
                ...
                elseif probe_type == 4 %tetrodes (repeat below)
                    ...
                    [fig1] = plot_tetrodes(spike_mat, spike_idx, dataset);
                    fig2= nan;
                 end
             end 
        ...
        else %make features from average of availabe spikes
            spike_idx_for_best_spikes = [];
            for ii = 1:num_spikes
                spike_idx_for_best_spikes = [spike_idx_for_best_spikes, spike_idx_sorted_by_spread(ii)];
            end 
...
function [fig1] = plot_tetrodes(spike_mat, spike_idx, dataset)
    %need the dimension of spikemat that corresponds to the number of eeg
    %channels. - i think its 1 and this should work. 
    num_eegs = size(spike_mat, 1); 
    cmap = colormap(bone(num_eegs));
    fig1 = figure();
    hold all
    for i = 1:num_eegs
...
function [max_amplitude, mean_amplitude, peak_to_trough_amplitude, peak_to_trough_slope]= make_DS2_features(spike_mat, spk_idx)
        ...
        for spike_num = spk_idx
            ...
            peak_to_trough_amplitude = peaks - troughs;
            peak_to_trough_time = trough_idx ./ 4800; %convert to seconds 
            peak_to_trough_slope = (troughs - peaks) ./ peak_to_trough_time; %calculate slope from peak to trough
%             peak_to_trough_slope = peak_to_trough_slope .* sign(peak_to_trough_amplitude); %this preserves the sign of the peak to trough difference 
