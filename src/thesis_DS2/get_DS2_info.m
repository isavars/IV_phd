...
% later in analysis. 
    ...
    [max_amplitude, mean_amplitude, fig1, fig2, ds2_rate, peak_to_trough_amplitude, peak_to_trough_slope, spike_mat] = get_DS2_from_egf( eeg_data, probe_type,sws_table);
    ...
    if probe_type == 1 
        ...
        for it_ch = 1:32 
            ...
            if max_amplitude(it_ch) < -0.5 
                DS2_info.DS2_labels(it_ch) = "above";
            elseif max_amplitude(it_ch) >= -0.5 && max_amplitude(it_ch) <= 0.5
                DS2_info.DS2_labels(it_ch) = "in";
            elseif max_amplitude(it_ch) > 0.5
    ...
    elseif probe_type ==4 
        ...
        eeg_chans = sleepData.eeg_channels{curr_trial};
        for it_ch = 1:32
            ...
            if it_ch  <= num_eegs
                DS2_info.channel(it_ch) = eeg_chans{it_ch};
            else 
