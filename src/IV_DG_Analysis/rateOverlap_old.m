...
%   age binning 
    ...
    for itAge=1:size(ageBins,1)
        ...
        for jj = 1: length(meanMeanRate)
            ...
            elseif (meanMeanRate(jj) <= 1.5) && (meanBurstIndex(jj) >= 0.10) && (corecorded(jj) > 3) %potential CA3 cells
                cluster4 = [cluster4; jj];
            elseif (meanMeanRate(jj) >= 0.02) && (meanBurstIndex(jj) >= 0.10) %potential mossy cells
