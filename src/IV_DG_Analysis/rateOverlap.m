...
%make rate change score and spatial information score
    ...
    for it_gma = 1: length (meanRate)
        ...
        SI_spat(it_gma,:) = max(SI_spat(it_gma,(1:5)));
    end
    ...
    for itAge=1:size(ageBins,1)
        ...
        novOverlap2 = [];
         for itC1 = 1: length(cluster2)
             if (nSpks(cluster2(itC1),FamInd(cluster2(itC1),1)) | nSpks(cluster2(itC1),FamInd(cluster2(itC1),2)) | nSpks(cluster2(itC1),NovInd(cluster2(itC1)))) >= 1
...
%             
        ...
        figure()
        y = mean(SI_spat(cluster2))
        errorC2 = transpose(std(SI_spat(cluster2))/sqrt(length(cluster2)));
        errors = errorC2
...
%         Environment = [repmat('Fam',length(famOverlap2),1);repmat('Nov', length(novOverlap2),1)];
         Environment = [Environment;repmat('Fam',length(famOverlap3),1);repmat('Nov', length(novOverlap3),1)];
