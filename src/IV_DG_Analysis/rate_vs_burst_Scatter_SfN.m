function rate_vs_burst_Scatter_SfN (spatData) 
...
% make spatiality score
    ...
    for it_SI = 1: length (meanRate)
        meanSI(it_SI,:) = mean(SI_spat(it_SI,1:5));
    end
    SI_spat = meanSI;
...
%     scatter(meanMeanRate(ExCluster), meanBurstIndex(ExCluster), 'b')
    scatter(meanMeanRate(cluster1), meanBurstIndex(cluster1),'r', 'LineWidth',2)
    scatter(meanMeanRate(cluster2), meanBurstIndex(cluster2), 'MarkerEdgeColor',[0.9290 0.6940 0.1250],'LineWidth',2)
    scatter(meanMeanRate(cluster3), meanBurstIndex(cluster3), 'MarkerEdgeColor',[0.4660 0.6740 0.1880],'LineWidth',3)
...
%     
    figure()
    scatter(meanBurstIndex(cluster1), SI_spat(cluster1), 'r', 'LineWidth',2)
    hold on;
        title('Spatiality Vs Burst Index')
        xlabel('Burst Index')
        ylabel('Spatial Score')
    scatter(meanBurstIndex(cluster2), SI_spat(cluster2),'MarkerEdgeColor',[0.9290 0.6940 0.1250],'LineWidth',2)
    hold on;
    scatter(meanBurstIndex(cluster3), SI_spat(cluster3), 'MarkerEdgeColor',[0.4660 0.6740 0.1880],'LineWidth',3)
    hold on;
    scatter(meanBurstIndex(cluster4), SI_spat(cluster4), 'k')
    scatter(meanBurstIndex(cluster5), SI_spat(cluster5), 'k')
    legend('Mystery Cells','Putative Granule','Putative Mossy', 'Unclassified');
    hold off;
