...
for ii=1:length(SD.selData)
    ...
    if strcmp(prms.envToInclude{1}, 'all' )
        ...
        for jj = 1:length(data.trials)
            ...
            NewEnvList = cell(1,5);
                ...
                elseif strcmp(data.trials(2).user.environment,trialOrder(5)) %&& strcmp(data.trials(4).user.environment,trialOrder(5))
                ...
                elseif strcmp(data.trials(1).user.environment,trialOrder(5))
    ...
    for jj=1:length(data_scaled.trials(1).cells)   %% In main loop, jj = iterator for cell %%
        ...
        for kk=1:length(trialsToUse)             %% In main loop, kk = iterator for trial %%
          if isempty(ResT.env(cellCount,kk)) || ResT.env(cellCount,kk) ~= trialOrder(5)  
            if prms.scaleToFitSquare
...
%             if nansum(dMap(:))==0;   continue;   end            
        ...
        elseif ResT.env(cellCount,kk) == trialOrder(5)
